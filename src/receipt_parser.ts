...
const DATA_INPUT_DIR = path.join(process.cwd(), "data", "input");
const DATA_GENERATED_DIR = path.join(process.cwd(), "data", "generated");
const RECEIPT_PATH = path.join(DATA_INPUT_DIR, "receipt.txt");
const PARSED_ITEMS_PATH = path.join(DATA_GENERATED_DIR, "parsed_items.json");
...
function ensureOutputDirs(): void {
  fs.mkdirSync(DATA_INPUT_DIR, { recursive: true });
...
function main(): void {
  ...
  if (!analysis.isItemized) {
    ...
    console.log("Run: npm run build-expenses");
    return;
  }
